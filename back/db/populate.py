...
client = MongoClient(
    f"mongodb://mongo1:27017/?directConnection=true"
)
...
counters_collection.create_indexes([IndexModel(["name"], unique=True)])
...
hash_algorithm_collection.create_indexes([IndexModel(["name"], unique=True)])
...
tag_collection.create_indexes([IndexModel(["tag"], unique=True)])
        ...
        "$jsonSchema": {
            ...
            "properties": {
                ...
                "externalLogin": {
                    ...
                    "additionalProperties": False,
                    "required": ["providerToken", "providerData"],
                    "properties": {
                        "providerToken": {
                            "bsonType": "string",
            ...
            "$or": [
                {
                    "$and": [
                        ...
                        {
                            "$not": {"$ifNull": ["$externalLogin", False]}
                        },  # B is not null
                    ]
                },
                {
...
def random_arr(
    generator, range_min=params["arr"]["min"], range_max=params["arr"]["max"]
):
    return [generator() for _ in range(random.randint(range_min, range_max))]


def random_unique_arr(
    generator, range_min=params["arr"]["min"], range_max=params["arr"]["max"]
):
    return list(set([generator() for _ in range(random.randint(range_min, range_max))]))


available_roles = {"verified": 0b1, "admin": 0b10, "premium": 0b100, "banned": 0b1000}
...
counter["_id"] = counters_collection.insert_one(counter).inserted_id
print("Done")

hash_algo_dicts = list()
for hash_algo in hash_algos:
    hash_algo_dicts.append({"name": hash_algo})
    if hash_algo == "argon2":
...
for user in users:
    ...
    user["sessions"] = [
        get_expiring_token(user["id"], "session") for _ in range(no_sessions)
    ]
    for session in user["sessions"]:
        session["_id"] = expiring_token_collection.insert_one(session).inserted_id

        session.pop("createdAt")
        session.pop("userId")

    user_collection.update_one(
        {"id": user["id"]}, {"$set": {"sessions": user["sessions"]}}
    )
    ...
    if changeType is not None:
        ...
        user_collection.update_one(
            {"id": user["id"]}, {"$set": {"login.changeToken": expiring_token}}
        )
...
for user in users:
    user["savedRecipes"] = [
        recipe["id"] for recipe in random.sample(recipes, k=random.randint(0, 3))
    ]
    user_collection.update_one(
        {"id": user["id"]}, {"$set": {"savedRecipes": user["savedRecipes"]}}
    )
...
# generate tags
tag_collection.insert_many([{"tag": k, "counter": v} for k, v in tags.items()], False)
print("Done")
...
for user in users:
    for follows in users:
        ...
        follow_collection.insert_one({"userId": user["id"], "followsId": follows["id"]})
print("Done")
...
for rating in ratings:
    ...
    while (
        recipe["authorId"] == user["id"]
        or (user["id"], recipe["id"]) in user_recipe_rating_combinations
    ):
        recipe = random_from(recipes)
        user = random_from(users)
...
for reply in replies:
    ...
    reply["_id"] = rating_collection.insert_one(reply).inserted_id
