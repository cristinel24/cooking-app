    ...
    def get_user_roles(self, user_id: str) -> int:
        try:
            with timeout(MAX_TIMEOUT_TIME_SECONDS):
                ...
                if not user:
                    raise RoleChangerException(status_code=status.HTTP_404_NOT_FOUND,
                                               error_code=ErrorCodes.NONEXISTENT_USER.value)
            ...
            raise RoleChangerException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                                       error_code=ErrorCodes.DB_ERROR.value)
